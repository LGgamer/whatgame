...
bool Monster::isDropItem(float prob)
{
	float ran =random(float(0), float(100));
	if (ran / 100 <= prob)
		return true;
	else
		return false;
...
void Monster::patrol(Node* player, Vec2 screenO)
{
	...
	else
	{
		...
		if (this->getPosition().x + x * 50 + screenO.x <= 0 || this->getPosition().x + x * 50 + screenO.x >= this->getParent()->getChildByName("Map")->getContentSize().width ||this->getPosition().y + y * 50 + screenO.y <=0 || this->getPosition().y + y * 50 + screenO.y >= this->getParent()->getChildByName("Map")->getContentSize().height)
		{
			x = random(-1, 1);
			y = random(-1, 1);
		}
...
void Monster::attack(Node* sprite, float delta)
{
	...
	auto str3 = this->getName().substr(7, std::string::npos);
	auto str2 = std::string("-bullet");
	auto str = damage + str2 + str1 + str3;
	bullet->setName(str);
...
void Boss_monster::attack(Node* sprite, float delta)//the boss monster will attack from eight dirction
{
	...
	auto bullet = Fly_monster::create("pumpkin.png");
	bullet->attackRange = 400;
	bullet->damage = 5;
	bullet->attackSpeed = 500;
	bullet->setScale(0.5);
	BulletNumber++;
	//log("%d", BulletNumber);
	bullet->setTag(2);
