...
bool GameStartPage::init() {
	...
	boxNumber = score = BulletNumber = soulNumber = soulNumberInMap = 0;
	...
	weaponbuff->setScale(0.1);
	this->addChild(weaponbuff);
	weaponbuff->setTag(7);
	weaponbuff->setName("attackup");

	auto weaponbuff2 = weapon1::create("speed.png");
	weaponbuff2->setPosition(Vec2(800, screenSize.height / 2));
	weaponbuff2->setScale(0.1);
	...
	sprite->basic_speed += UserDefault::getInstance()->getIntegerForKey("speedUp");
	sprite->basic_health += UserDefault::getInstance()->getIntegerForKey("hpUp");
	MAXhealth = sprite->basic_health;
	sprite->attackM = 1;
	...
	if (isEffect==true)
		audioengine->playBackgroundMusic("sound/firstPhase.mp3", true);
	...
	scheduleOnce(schedule_selector(GameStartPage::delay), 2);
    

    return true;
}

void GameStartPage::delay(float dt)
{
	this->scheduleUpdate();
...
void GameStartPage::update(float delta) {
	...
	//check the bullet whether it kills the monsters
	checkBullet();
	...
	if (isKeyPressed(EventKeyboard::KeyCode::KEY_A)) 
	{
		...
		if (cpm.x - pwith*0.5 <= 0)
			...
			if (hitted.height > 0 && hitted.width > 0)//, monsterP.x, monsterP.y, mwith, mheig))
			{
				...
				{
					if (hittedSprite->getName().find("attackup") != std::string::npos) {
						weapon1* weapon1buff = new  weapon1;
						weapon1buff->effect(maincharacter);
						delete weapon1buff;
					}
					else if (hittedSprite->getName().find("speedup") != std::string::npos) {

						weapon2 * weaponbuff2 = new weapon2;
						weaponbuff2->effect(maincharacter);
						delete weaponbuff2;
					}
					else if (hittedSprite->getName().find("waepon3") != std::string::npos)
					{
						weapon3 * weaponbuff3 = new weapon3;
						weaponbuff3->effect(maincharacter);
						delete weaponbuff3;
					}
					else if (hittedSprite->getName().find("soul") != std::string::npos)
					{
						soulNumber++;
					}
...
void GameStartPage::fire(float dt) {
	//auto sprite = (character*) this->getChildByTag(1);

	if (isKeyPressed(EventKeyboard::KeyCode::KEY_DOWN_ARROW)) 
	{
		SimpleAudioEngine::getInstance()->playEffect("sound/firesound.wav");
		switch (sprite->attackM)
		{
		case 1:
			sprite->fire1(Vec2(0, -sprite->getContentSize().height / 10));
			break;
		case 2:
			sprite->fire2(Vec2(0, -sprite->getContentSize().height / 10));
			break;
		default:
			sprite->fire1(Vec2(0, -sprite->getContentSize().height / 10));
			break;
		...
		{
		case 1:
			sprite->fire1(Vec2(sprite->getContentSize().width / 10,0));
			break;
		case 2:
			sprite->fire2(Vec2(sprite->getContentSize().width / 10, 0));
			break;
		default:
			sprite->fire1(Vec2(sprite->getContentSize().width / 10, 0));
			break;
...
void GameStartPage::updateScore()
{
	...
	scoreNumber->setPosition(Vec2(screenSize.width - 150, screenSize.height/2 + 30));
	scoreNumber->setTag(4);
	auto remove = RemoveSelf::create();
	this->getChildByName("HUD")->addChild(scoreNumber);
...
void GameStartPage::dropSoul(Vec2 position)
{
	...
	soul->setTag(7);
	soulNumberInMap++;
	auto str1 = std::to_string(soulNumberInMap);
...
void GameStartPage::initWall()
{
	...
	wall_1->setPosition(Vec2(screenSize.width / 2 - 1000, screenSize.height / 2 + 1000));
	wall_1->setTag(3);
	wall_1->setScale(1);
	...
	wall_3->setPosition(Vec2(screenSize.width / 2 - 850, screenSize.height / 2 - 1350));
	wall_3->setTag(3);
	wall_3->setScale(1);
...
#if 1
	auto testMonster = Sprite::create("elements/game.png");
	...
	auto testMonster2 = Sprite::create("elements/gameCenter.png");
	testMonster2->setPosition(Vec2(screenSize.width / 2 + 840, screenSize.height / 2 + 1000));
	testMonster2->setScale(0.2);
	testMonster2->setTag(3);
	testMonster2->setName("Wall_5");
	this->addChild(testMonster2);

	auto testMonster3 = Sprite::create("elements/gameCenter2.png");
	testMonster3->setPosition(Vec2(screenSize.width / 2 + 1000, screenSize.height / 2-1000));
	testMonster3->setScale(0.2);
	testMonster3->setTag(3);
	testMonster3->setName("Wall_4");
	this->addChild(testMonster3);

	auto testMonster4 = Sprite::create("elements/gameCenter.png");
	testMonster4->setPosition(Vec2(screenSize.width / 2 + 100, screenSize.height / 2-600));
	testMonster4->setScale(0.2);
...
void GameStartPage::write()
{
	...
	userDefault->setIntegerForKey("soulForthisTurn", soulNumber);
	userDefault->setIntegerForKey("player_attackM", sprite->attackM);
	userDefault->setIntegerForKey("player_attack", sprite->get_attack());
	userDefault->setIntegerForKey("player_attackRange", sprite->getAttackRange());
	userDefault->setIntegerForKey("player_attackSpeed", sprite->getAttackSpeed());
	userDefault->setIntegerForKey("speedUp", 0);
	userDefault->setIntegerForKey("player_Speed", sprite->get_speed());

	userDefault->setIntegerForKey("hpUp", 0);
	userDefault->setIntegerForKey("player_HP", sprite->get_current_health());
	userDefault->setIntegerForKey("player_MAXHP", MAXhealth);
	

	if (userDefault->getIntegerForKey("soulForAll") == false) {
		userDefault->setIntegerForKey("soulForAll", soulNumber);
	}
	else {
		userDefault->setIntegerForKey("soulForAll", userDefault->getIntegerForKey("soulForAll") + soulNumber);
	}
...
{
	return UserDefault::getInstance()->getIntegerForKey("soulForthisTurn");
}

void GameStartPage::initBox()
{
	int boxN = random(1, cround);
	...
	std::vector<int> xList = RandomList::randmInt(10, boxN);
	std::vector<int> yList = RandomList::randmInt(10, boxN);
	const int INTERVAL = 50;

	for (int i = 0; i < boxN; i++)
	{
		auto box = Monster::create("box.png");

		box->setPosition(Vec2(mapSize.width / 2 + 1000 + xList[i] * INTERVAL - screenO.x, mapSize.height - 800 + yList[i] * INTERVAL - screenO.y));
		box->setScale(1);
		box->setTag(8);
...
void GameStartPage::dropBuff(Vec2 pos)
{
	auto buff = weapon3::create("attackup.png");
	buff->setPosition(pos);
	buff->setScale(0.3);
	buff->setTag(7);
	buff->setName("waepon3");
	this->addChild(buff);
}
