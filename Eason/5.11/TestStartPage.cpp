...
bool GameStartPage::init() {
	...
	auto Score = Label::createWithSystemFont("Score", "Arial", 86);
	Score->setPosition(Vec2(screenSize.width - 150, screenSize.height - 100));
	Score->setTag(4);
	this->addChild(Score);
...
void GameStartPage::monsterAttack(float delta)
{
	...
	for (Node* i : childList)
	{
		if (i->getTag() == 2)
		{
			...
			{
				log("pumpkin_pumpkin_pumpkin_pumpkin_pumpkin_pumpkin_pumpkin_");
				monster = (Fly_monster*) monster;
			}
			else
			{
