...
bool DryDew::init()
{
	...
	dd->setPosition(Vec2(Director::getInstance()->getVisibleSize().width / 2, Director::getInstance()->getVisibleSize().height / 2 +225));
	this->addChild(dd);

	//press any key to continue
	auto pak = Sprite::create("PAKTC.png");
	pak->setPosition(Vec2(Director::getInstance()->getVisibleSize().width / 2, Director::getInstance()->getVisibleSize().height / 2 - 300));
	pak->setScale(0.5);
